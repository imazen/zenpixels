...
- Versioned public-API surface snapshots at `docs/public-api/{zenpixels,zenpixels-convert}.txt`, regenerated by `zenpixels/tests/public_api_doc.rs` on every `cargo test` (`ZEN_API_DOC=check` verifies in CI's clippy job, `=off` skips; `just api-doc` / `just api-doc-check` locally).

### zenpixels — added

- **`PixelDescriptor::RGB16_BT2100_PQ` / `RGB16_BT2100_HLG` presets + `new`↔`new_full` doc cross-references** (#28) — BT.2100 HDR descriptors (BT.2020 primaries, PQ/HLG transfer, full range — CICP `(9, 16|18, 0, full)`) without spelling out the enums, and `# See also` sections so `new`'s docs surface `new_full` for non-default primaries (and vice versa).
- **`PixelDescriptor::bytes_per_channel()`** (#29) — per-sample byte width (`channel_type().byte_size()` shorthand); the usual high-bit-depth gate is `bytes_per_channel() > 1`.
- **Endianness contract documented on `PixelSlice::new` / `PixelSliceMut::new`** (#29) — multi-byte samples (U16/F16/F32) are interpreted in native byte order; `ByteOrder` is *channel* order (BGR vs RGB), not byte order within a sample, so decoders reading big-endian container data must swap before wrapping. (Issue #29's constructor-validation option targets an endianness tag that doesn't exist on `PixelDescriptor`; adding one is spec work, tracked with the #36 cluster.)
...
- `orient::apply_orientation_in_place(PixelSliceMut, Orientation) -> Result<PixelSliceMut, ConvertError>` — bakes orientation **reusing the buffer's own allocation**, with no second pixel buffer (the transposing orientations would otherwise hold a 2× transient). Consumes the view, permutes the bytes in place (square transpose = diagonal swap; non-square = cycle-following with an `n`-element visited scratch — not a 2× buffer; flips = row/element swaps; padded input is compacted to tight first), and returns a re-described tight-stride view (dims swapped for the four transposing orientations; the source `PixelBuffer`'s own dims go stale, like the reduce-in-place APIs). Trade-off: non-square transpose uses cycle-following (cache-hostile, no SIMD) so it's slower than the out-of-place SIMD path — choose it when peak memory matters more than speed. `bpp > 16` returns `ConvertError::BufferSize`. Parity-gated against the out-of-place path across square/non-square × tight/padded × 5 formats × all 8 orientations.
- **`adapt::try_adapt_in_place(PixelSliceMut, PixelDescriptor) -> Result<PixelSliceMut, PixelSliceMut>`** (#11) — allocation-free format adaptation for same-byte-size transitions: metadata-only re-tags (same format, new transfer/primaries/range/alpha tags) move zero bytes, and `Rgba8`↔`Bgra8` (X-padding forms included) runs garb's SIMD B↔R swap in the existing buffer, strided rows handled. `Err` returns the slice **unchanged** so callers fall through to the allocating `adapt_for_encode`; shape-changing transitions (channel count, bit depth) always `Err`. Eliminates the full-frame allocation for BGRA-native pipelines (imageflow) feeding RGBA-preferring encoders. Also handles **contract-exact alpha-lane removal** (RGBA→RGB, BGRA→RGB with the B↔R reorder, GrayAlpha→Gray, at U8/U16/F32 as formats exist) when the source alpha mode is `Undefined` (X padding) or declared `Opaque`: rows compact in place with the stride rounded down to a whole number of target pixels — when the input stride already divides evenly, rows stay at their own bases (zero cross-row movement; freed bytes become row padding). Live `Straight`/`Premultiplied` alpha still `Err`s (a blind discard would silently diverge from `adapt_for_encode`'s matting policy; *measured*-opaque alpha is `reduce_to_load_bearing_format_in_place`'s job).
