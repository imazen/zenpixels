...
- **`synthesize_icc_for_cicp` now gives full CICP→ICC coverage from a bundled compressed blob — no CMS required.** A default (no-feature) build resolves a profile for the entire assigned ITU-T H.273 grid (11 primaries × 16 transfers = 174 profile-yielding combos, the 2 sRGB-default pairs excluded as `NotNeeded`), where it previously returned `NeedsCms` for anything outside the 4 `&'static` consts. The profiles are bundled as one transfer-grouped LZ4-HC asset (`src/profiles/cicp_bundle.lz4`, ~27 KB; 596 KB raw → 21.8×) generated build-time by the `icc-gen` `cicp_bundle_gen` tool from moxcms; the runtime decodes only the touched transfer group (lazily, once, into a per-group cache via the existing `once_cell::race::OnceBox`) and slices the profile out as a zero-copy `Cow::Borrowed`. Decode is pure-Rust `no_std` `lz4_flex` `safe-decode` — `#![forbid(unsafe_code)]` holds. This makes moxcms a **build-time generator only**: `cms-moxcms` is no longer required for full coverage (it stays available as an optional alternative). Internal implementation detail — `synthesize_icc_for_cicp`'s signature and the `SynthesizedIcc` enum are unchanged. A no-CMS build reaching the fallback now means the CICP is outside the assigned grid (a reserved code point), so it reports `CmsUnsupported` rather than `NeedsCms`. The committed blob is reproducible (deterministic regen; ICC creation-timestamp zeroed) and pinned by a golden sha256 test; a `cms-moxcms`-gated test asserts every bundled profile is byte-identical to a fresh moxcms generation. (fcc173fa, 5854a217, 3d6f4d29)
