...
pub(crate) fn convert_f32_rgb_dispatch(
    ...
    match (src_trc, dst_trc) {
        (Srgb, Srgb) => {
            incant!(convert_rgb_srgb(m, data), [-v4]);
            return true;
        }
        (Bt709, Bt709) => {
            incant!(convert_rgb_bt709(m, data), [-v4]);
            return true;
        }
        (Pq, Pq) => {
            incant!(convert_rgb_pq(m, data), [-v4]);
            return true;
        }
        (Hlg, Hlg) => {
            incant!(convert_rgb_hlg(m, data), [-v4]);
            return true;
        }
        (Gamma22, Gamma22) => {
            incant!(convert_rgb_adobe(m, data), [-v4]);
            return true;
        }
        (Pq, Srgb) => {
            incant!(convert_rgb_pq_to_srgb(m, data), [-v4]);
            return true;
        }
        (Hlg, Srgb) => {
            incant!(convert_rgb_hlg_to_srgb(m, data), [-v4]);
            return true;
        }
        (Srgb, Pq) => {
            incant!(convert_rgb_srgb_to_pq(m, data), [-v4]);
            return true;
        }
        (Bt709, Srgb) => {
            incant!(convert_rgb_bt709_to_srgb(m, data), [-v4]);
            return true;
        }
        (Srgb, Bt709) => {
            incant!(convert_rgb_srgb_to_bt709(m, data), [-v4]);
            return true;
        }
        (Gamma22, Srgb) => {
            incant!(convert_rgb_adobe_to_srgb(m, data), [-v4]);
            return true;
        }
        (Srgb, Gamma22) => {
            incant!(convert_rgb_srgb_to_adobe(m, data), [-v4]);
            return true;
        }
...
pub(crate) fn convert_f32_rgba_dispatch(
    ...
    match (src_trc, dst_trc) {
        (Srgb, Srgb) => {
            incant!(convert_rgba_srgb(m, data), [-v4]);
            return true;
        }
        (Bt709, Bt709) => {
            incant!(convert_rgba_bt709(m, data), [-v4]);
            return true;
        }
        (Pq, Pq) => {
            incant!(convert_rgba_pq(m, data), [-v4]);
            return true;
        }
        (Hlg, Hlg) => {
            incant!(convert_rgba_hlg(m, data), [-v4]);
            return true;
        }
        (Gamma22, Gamma22) => {
            incant!(convert_rgba_adobe(m, data), [-v4]);
            return true;
        }
        (Pq, Srgb) => {
            incant!(convert_rgba_pq_to_srgb(m, data), [-v4]);
            return true;
        }
        (Hlg, Srgb) => {
            incant!(convert_rgba_hlg_to_srgb(m, data), [-v4]);
            return true;
        }
        (Srgb, Pq) => {
            incant!(convert_rgba_srgb_to_pq(m, data), [-v4]);
            return true;
        }
        (Bt709, Srgb) => {
            incant!(convert_rgba_bt709_to_srgb(m, data), [-v4]);
            return true;
        }
        (Srgb, Bt709) => {
            incant!(convert_rgba_srgb_to_bt709(m, data), [-v4]);
            return true;
        }
        (Gamma22, Srgb) => {
            incant!(convert_rgba_adobe_to_srgb(m, data), [-v4]);
            return true;
        }
        (Srgb, Gamma22) => {
            incant!(convert_rgba_srgb_to_adobe(m, data), [-v4]);
            return true;
        }
        _ => {}
    }
...
pub(crate) fn convert_u16_rgb_simd_matlut(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        incant!(convert_u16_rgb_matlut(m, src, dst, lin_lut, enc_lut), [-v4]);
        return;
    }
    #[cfg(not(target_arch = "x86_64"))]
...
mod hybrids {
    ...
    pub(crate) fn convert_u16_rgb_simd_lutdec_polyenc(
        ...
        #[cfg(target_arch = "x86_64")]
        {
            incant!(convert_u16_rgb_lutdec_polyenc(m, src, dst, lin_lut), [-v4]);
            return;
        }
        #[cfg(not(target_arch = "x86_64"))]
    ...
    pub(crate) fn __bench_u16_poly_decode_lut_encode(
        ...
        #[cfg(target_arch = "x86_64")]
        {
            incant!(convert_u16_rgb_polydec_lutenc(m, src, dst, enc_lut), [-v4]);
            return;
        }
        #[cfg(not(target_arch = "x86_64"))]
    ...
    pub(crate) fn __bench_u16_poly_decode_poly_encode(
        ...
        #[cfg(target_arch = "x86_64")]
        {
            incant!(convert_u16_rgb_polydec_polyenc(m, src, dst), [-v4]);
            return;
        }
        #[cfg(not(target_arch = "x86_64"))]
...
pub(crate) fn convert_u8_to_f32_lin_simd(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        incant!(convert_u8_to_f32_lin(m, src, dst, lin_lut), [-v4]);
        return;
    }
    #[cfg(not(target_arch = "x86_64"))]
...
pub(crate) fn convert_f32_lin_to_u8_simd(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        incant!(convert_f32_lin_to_u8(m, src, dst, enc_lut), [-v4]);
        return;
    }
    #[cfg(not(target_arch = "x86_64"))]
...
pub(crate) fn convert_u8_rgb_simd_matlut(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        incant!(convert_u8_rgb_matlut(m, src, dst, lin_lut, enc_u8), [-v4]);
        return;
    }
    #[cfg(not(target_arch = "x86_64"))]
