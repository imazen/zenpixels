...
### linear-srgb (unblocker — DONE locally via path dep)

The `tf::*::*_x16<T: F32x16Convert>` and `tf::*::*_x4<T: F32x4Convert>`
families were `pub(crate)`. The visibility flip is shipped in the
sibling worktree `../linear-srgb--pub-tf-x16` and activated via path
dep in `zenpixels-convert/Cargo.toml`:

```toml
linear-srgb = { path = "../../linear-srgb--pub-tf-x16",
                default-features = false,
                features = ["transfer", "avx512"] }
```

What was flipped:
- `tf/mod.rs`: `pub(crate) mod {bt709,fast_math,hlg,pq,srgb}` → `pub mod`
- `tf/srgb.rs`: `_x4`/`_x8`/`_extended_x16` pairs → `pub fn`. Added
  clamped `srgb_to_linear_x16<T: F32x16Convert>` /
  `linear_to_srgb_x16<T: F32x16Convert>` parallel to the x8 versions.
- `tf/bt709.rs`, `tf/pq.rs`, `tf/hlg.rs`: `_x4`/`_x8`/`_x16` → `pub fn`.

Internal helpers in `tf/fast_math.rs` (eval_rational_poly_x{4,8,16},
fast_log2f_x{4,8,16}, fast_pow2f_x{4,8,16}, fast_powf_x{4,8,16}) and
the `pq.rs` polynomial constants stay `pub(crate)` — visibility doesn't
propagate, so the now-`pub` TF entry points can still call them.

Upstream PR is **not yet open**. Path dep is sufficient for
benchmarking the redesign. PR window: after the redesign is proven on
benchmarks, prepare a clean PR with just the visibility flip + clamped
x16 sRGB addition, no zenpixels-convert content.
