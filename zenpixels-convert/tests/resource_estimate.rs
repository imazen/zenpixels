//! Tests for the `(u64, f64)` estimate primitive on `ConvertPlan`.
//!
...
//!
//! Pre-publish YAGNI cleanup: the over-elaborated `ResourceEstimate` /
//! `StepEstimate` / `EstimateConfidence` surface that briefly existed at
//! 0.2.15 collapsed into a single tuple-returning `ConvertPlan::estimate`.
//! These tests pin the shape that survives.

use zenpixels::AlphaMode;
use zenpixels_convert::{
    ChannelType, ColorPrimaries, ConvertPlan, PixelDescriptor, TransferFunction,
};
...
// ---------------------------------------------------------------------------
// (2) Zero-pixel plan: trivially returns (0, 0).
// ---------------------------------------------------------------------------

#[test]
fn zero_pixel_plan_returns_zero_tuple() {
...
fn peak_memory_at_least_destination_buffer() {
    ...
    assert!(
        mem >= dst_bytes,
        "peak_memory_bytes {mem} < dst_bytes {dst_bytes}",
    );
}
